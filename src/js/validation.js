document.addEventListener("change", event => {
  ...
  if (closest !== null && "data-rules-element" in closest.attributes && "data-column-id" in closest.attributes) {
    ...
    fetch(new Request(document.body.attributes["data-baseurl"].value + "/api/validation?id=" + encodeURIComponent(closest.attributes["data-column-id"].value) + "&value=" + encodeURIComponent(value)), {redirect: "manual"})
      .then(response => {
        if (response.ok) {
